...
describe("Settings API (Integration)", () => {
  ...
  const createBoardingTypePayload = () => ({
    cmbStart: 100,
    cmbEnd: 200,
  ...
  describe("BoardingTypesController", () => {
    const simpleGetEndpoints = [
      { path: "/api/settings/boarding-types", description: "list boarding types", expectArray: true },
      { path: "/api/settings/boarding-types/active", description: "list active boarding types", expectArray: true },
      { path: "/api/settings/default-boarding-type", description: "get default boarding type", expectArray: false },
    ];

    simpleGetEndpoints.forEach(({ path, description, expectArray }) => {
      describe(`GET ${path}`, () => {
    ...
    describe("GET /api/settings/boarding-types/:id", () => {
      it("should get boarding type details successfully", async () => {
        const createRes = await ctx.authReq.post("/api/settings/boarding-types").send(createBoardingTypePayload());
    ...
    describe("POST /api/settings/boarding-types", () => {
      it("should create boarding type successfully", async () => {
        const res = await ctx.authReq.post("/api/settings/boarding-types").send(createBoardingTypePayload());
      ...
      it("should return 401 without auth", async () => {
        await ctx.req.post("/api/settings/boarding-types").send({ brazilExpenses: 1000.0 }).expect(401);
      });
    });
    ...
    describe("PATCH /api/settings/boarding-types/:id", () => {
      it("should update boarding type successfully", async () => {
        ...
        const res = await ctx.authReq
          .patch(`/api/settings/boarding-types/${createRes.body.id}`)
          .send({ brazilExpenses: 2000.0 });
        expect(res.status).toBe(200);
      });
      ...
      it("should return 400 with invalid payload", async () => {
        ...
        await ctx.authReq
          .patch(`/api/settings/boarding-types/${createRes.body.id}`)
          .send({ cmbStart: "invalid" })
          .expect(400);
      });

      it("should return 401 without auth", async () => {
        await ctx.req.patch("/api/settings/boarding-types/test-id").send({ brazilExpenses: 2000.0 }).expect(401);
      });
    });
  ...
  describe("FreightsController", () => {
    ...
    describe("GET /api/settings/freights/:id", () => {
      it("should get freight details successfully", async () => {
        const createRes = await ctx.authReq.post("/api/settings/freights").send(createFreightPayload());
    ...
    describe("POST /api/settings/freights", () => {
      it("should create freight successfully", async () => {
        const res = await ctx.authReq.post("/api/settings/freights").send(createFreightPayload());
        expect(res.status).toBe(201);
        expect(res.body).toHaveProperty("id");
      });
      ...
      it("should return 401 without auth", async () => {
        await ctx.req.post("/api/settings/freights").send({ destino: "São Paulo", uf: "SP" }).expect(401);
      });
    });
    ...
    describe("PATCH /api/settings/freights/:id", () => {
      it("should update freight successfully", async () => {
        const createRes = await ctx.authReq.post("/api/settings/freights").send(createFreightPayload());
        const res = await ctx.authReq.patch(`/api/settings/freights/${createRes.body.id}`).send({ peso10: 6.0 });
        expect(res.status).toBe(200);
      });
      ...
      it("should return 400 with invalid payload", async () => {
        const createRes = await ctx.authReq.post("/api/settings/freights").send(createFreightPayload());
        await ctx.authReq
          .patch(`/api/settings/freights/${createRes.body.id}`)
          .send({ uf: "INVALID_UF_TOO_LONG" })
          .expect(400);
      });

      it("should return 401 without auth", async () => {
        await ctx.req.patch("/api/settings/freights/1").send({ peso10: 6.0 }).expect(401);
      });
    });
    ...
    describe("POST /api/settings/frete/calcular", () => {
      ...
      it("should return 401 without auth", async () => {
        await ctx.req
          .post("/api/settings/frete/calcular")
          .send({ origin: "Guangzhou", destination: "São Paulo", weight: 1000 })
          .expect(401);
      });
    });
  });
